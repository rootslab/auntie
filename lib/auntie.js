...
exports.Auntie = ( function () {
    ...
    aproto.count = function ( data ) {
        ...
        if ( ! cslen ) {
            ...
            let [ cmatches, rbytes ] = bop.scount( data );
            // save remaining bytes
            me.csnip = cmatches ? data.slice( dlen - rbytes ) : data;
            // update internal counter and return matches
            return [ cnt[ 0 ] += cmatches ];
        }
        ...
        let csnip = bconcat( [ me.csnip, data.slice( 0, plen ) ] )
            ...
            , [ cmatches, rbytes ] = bop.scount( csnip, offset )
            ;
        ...
        if ( cmatches ) {
            ...
            if ( rbytes ) {
                ...
                [ cmatches, rbytes ] = bop.scount( data, plen - rbytes );
                if ( cmatches ) {
                    me.csnip = data.slice( dlen - rbytes );
                    return [ cnt[ 0 ] += cmatches ];
                }
            ...
            [ cmatches, rbytes ] = bop.scount( data, plen );
            if ( cmatches ) {
                me.csnip = data.slice( dlen - rbytes );
                return [ cnt[ 0 ] += cmatches ];
            }
        ...
        if ( dlen > plen ) {
            [ cmatches, rbytes ] = bop.scount( data );
