...
exports.Auntie = ( function () {
    ...
    aproto.do = function ( data, collect ) {
        ...
        if ( slen ) {
            ...
            let snip = bconcat( [ me.snip, data.slice( 0, plen - 1 ) ] )
                ...
                , match = bop.sparse( snip, snip.length - plen - 1) // + 1
                ;
    // log( 'snip', snip.slice( snip.length - plen ) )
            // only 1 match at max is possible
            if ( match.length ) {
                let m = match[ 0 ]
                    ;
    //log( '\n- BETWEEN/SNIP:', snip.slice( 0, m ) + '')
                fn( m ? snip.slice( 0, m ) : snip );
    ...
    // log( '\n- OFFSET/DATA:', offset, data + '' )
        ...
        if ( plen > data.length - offset ) 
            me.snip = data.slice( offset );
        else {
    ...
    aproto.count = function ( data ) {
        ...
        let offset = 0
            ...
            ;
        if ( cslen ) {
            // a snip of previous data exists, slice only necessary bytes
            let snip = bconcat( [ me.csnip, data.slice( 0, plen ) ] )
                // difference could be negative
                , diff = cslen - plen
                ;
            // parse from the next possible index
            result = bop.scount( snip, snip.length - plen + 1 );
            // only 1 match at max is possible, in that case, change offset to the last possible index
            if ( result[ 0 ] && ++me.cnt[ 0 ] ) offset = snip.length - result[ 1 ] - cslen;              
