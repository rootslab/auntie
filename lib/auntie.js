...
- where: #do and #dist
- problem: no match for several chunks.
- then: the snip will become very large.

- note: we could bufferize, collecting multiple buffers
        which don't contain sequence, therefore delaying
        the data concatenation; however, for now, we 
        concat onlythe previous chunk of data with the
        current.
...
exports.Auntie = ( function () {
    ...
    aproto.do = function ( data, collect ) {
        ...
        let matches = split( data, offset, fn )
            , mlen = matches.length
            ;
