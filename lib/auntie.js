...
/**/
    aproto.count = function ( data ) {
        ...
        if ( plen > cslen + dlen ) {
            // no enough data to parse the sequence
            me.csnip = bconcat( [ me.csnip, data ] );
            return [ cnt[ 0 ] ];
        }
        
        // enough data to find the sequence ( plen <= cslen + dlen )
        offset = cslen - plen + 1;
        ...
        // slice only necessary bytes (= plen - 1 )
        // let csnip = bconcat( [ me.csnip, data.slice( 0, plen - 1 ) ] )
        //    ;
        // count from the current offset
        //result = bop.scount( csnip, offset );

       //log( '\n--> OLD: \n(%d) %s', me.csnip.length, me.csnip );
       //log( '\n--> DATA:\n(%d) %s', data.length, data )
       //log( '\n--> SNIP:\n(%d) %s\n offset:%d, res: %d, rem: %d, chunk length: %d, curr snip length: %d\n',
       //     csnip.length, csnip, offset, result[ 0 ], result[ 1 ], data.length );
        ...
        // parse only new received data
        result = bop.scount( data );
        if ( result[ 0 ] ) {
            me.csnip = data.slice( dlen - result[ 1 ] );
            [ cnt[ 0 ] += result[ 0 ] ];
        } else {
            // no results parsing new data, concat buffers? no TODO
            me.csnip = bconcat( [ me.csnip, data ] );
            // me.csnip =  Buffer.allocUnsafe( 0 );
            return [ cnt[ 0 ] ];
        }
...
/** /
    aproto.count = function ( data ) {
        ...
        if ( cslen ) {
            // a snip of previous data exists
            if ( plen <= ( me.csnip.length + data.length ) ) {
                // slice only necessary bytes
                let csnip = bconcat( [ me.csnip, data.slice( 0, plen - 1 ) ] )
                    ;
                result = bop.scount( csnip, cslen - plen + 1 );
                // only 1 match at max is possible
                if ( result[ 0 ] ) {
                    //  increment counter
                    ++me.cnt[ 0 ];
                    // parse remaining bytes
                    if ( result[ 1 ] ) {
                        // offset for the current data (0 to plen - 1)
                        result = bop.scount( data, data.length - result[ 1 ]);
                        me.csnip = result[ 0 ] ? data.slice( data.length - result[ 1 ] ) : data;
                        return [ cnt[ 0 ] += result[ 0 ] ];
                    }
                    // no enough data, 0 bytes to parse
                    me.csnip = Buffer.from( [] );
                    return [ me.cnt[ 0 ] ];
                }
                // parse only new received data
                data = bconcat( [ me.csnip, data ] );
                 // negative offsets will be set to 0 by Bop
                result = bop.scount( data, cslen - plen + 1 );
                me.csnip = result[ 0 ] ? data.slice( data.length - result[ 1 ] ) : data;
                return [ cnt[ 0 ] += result[ 0 ] ];
            } else {
