...
exports.Auntie = ( function () {
    ...
    aproto.count = function ( data ) {
        ...
        if ( ! cslen ) {
            ...
            let [ mcnt, bytes ] = bop.scount( data );
            // save remaining bytes
            me.csnip = mcnt ? data.slice( dlen - bytes ) : data;
        ...
        let csnip = bconcat( [ me.csnip, data.slice( 0, plen ) ] )
            ...
            , [ mcnt, bytes ] = bop.scount( csnip, offset )
            ;
        ...
        if ( mcnt ) {
            ...
            if ( bytes ) {
                ...
                [ mcnt, bytes ] = bop.scount( data, plen - bytes );
                if ( mcnt ) {
                    me.csnip = data.slice( dlen - bytes );
                    return [ cnt[ 0 ] += mcnt ];
                }
            ...
            [ mcnt, bytes ] = bop.scount( data, plen );
            if ( mcnt ) {
                me.csnip = data.slice( dlen - bytes );
                return [ cnt[ 0 ] += mcnt ];
            }
