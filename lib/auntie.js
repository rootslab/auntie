...
exports.Auntie = ( function () {
    ...
    aproto.count = function ( data ) {
        ...
        if ( result[ 0 ] ) {
            ...
            // log ( '%s %s -> %s', me.csnip, data, csnip, offset, result  )
            // no enough data in the concat snip, discard previous snip
            // parse only the new data from the first possible offset
            // data = bconcat( [ me.csnip, data ] );
            result = bop.scount( data, plen - 1 );
            if ( result[ 0 ] ) {
                me.csnip = data.slice( dlen - result[ 1 ] );
                return [ cnt[ 0 ] += result[ 0 ] ];
            }
            // save the snip, slicing bytes already parsed  
            me.csnip = data.slice( plen - 1 );
            return [ cnt[ 0 ] ];
        }
